...
namespace LXC
{
    ...
    class Container
    { // TODO Rewrite to Napi::ObjectWrap
    private:
    ...
    public:
        ...
        bool create(const char *t, const char *bdevtype, struct bdev_specs *specs, int flags, char **const argv)
        {
        ...
        char *get_keys(const char *key)
        {
            ...
            int len = this->container->get_keys(this->container, key, nullptr, 0);
            if (len <= 0)
                return nullptr;
        again:
            value = (char *)malloc(sizeof(char) * len + 1);
