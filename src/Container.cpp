...
{
    assert(_container, "Invalid container pointer") return Napi::String::New(info.Env(), _container->name);
}
...
{
    assert(_container, "Invalid container pointer") return Napi::String::New(info.Env(), _container->state(_container));
}

Napi::Value Container::GetRunning(const Napi::CallbackInfo &info)
{
    assert(_container, "Invalid container pointer") return Napi::Boolean::New(info.Env(), _container->is_running(_container));
}

Napi::Value Container::GetInitPID(const Napi::CallbackInfo &info)
{
    assert(_container, "Invalid container pointer") return Napi::Number::New(info.Env(), _container->init_pid(_container));
}

Napi::Value Container::GetConfigFileName(const Napi::CallbackInfo &info)
{
    assert(_container, "Invalid container pointer") return Napi::String::New(info.Env(), _container->config_file_name(_container));
}

Napi::Value Container::GetDefined(const Napi::CallbackInfo &info)
{
    assert(_container, "Invalid container pointer") return Napi::Boolean::New(info.Env(), _container->is_defined(_container));
}
...
Napi::Value Container::Freeze(const Napi::CallbackInfo &info)
{
    ...
    assert_deferred(_container, "Invalid container pointer") auto worker = new AsyncPromise<>(
        deferred,
...
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer") auto worker = new AsyncPromise<>(deferred, [this](AsyncPromise<> *worker)
                                                                                              {
...
Napi::Value Container::LoadConfig(const Napi::CallbackInfo &info)
{
    ...
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") auto alt_file = info[0].ToString().Utf8Value();
...
{
    assert(_container, "Invalid container pointer") return Napi::Boolean::New(info.Env(), _container->daemonize);
}

Napi::Value Container::GetConfigPath(const Napi::CallbackInfo &info)
{
    assert(_container, "Invalid container pointer") return Napi::String::New(info.Env(), _container->get_config_path(_container));
}
...
Napi::Value Container::WantCloseAllFds(const Napi::CallbackInfo &info)
{
    assert(_container, "Invalid container pointer")
        check(info.Length() <= 0 || !info[0].IsBoolean(), "Invalid arguments") return Napi::Boolean::New(info.Env(), _container->want_close_all_fds(_container, info[0].ToBoolean()));
}
        ...
        [this, force, include_snapshots](AsyncPromise<> *worker)
        {
            ...
            if (_container->is_defined(_container))
            {
                ...
                if (ret > 0 && strcmp(buf, "0") == 0)
                {
                    ...
                    {
                        if (!_container->destroy_with_snapshots(_container)) //TODO: THIS DOESN'T WORK YET MAYBE BECAUSE CONTAINER HAS NO SNAPSHOT???
                        {
...
{
    assert(_container, "Invalid container pointer")
        check(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") return Napi::String::New(info.Env(),
...
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container point") int _ttynum = info[0].IsNumber() ? info[0].ToNumber().Int32Value() : -1;
...
Napi::Value Container::Snapshot(const Napi::CallbackInfo &info)
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") auto commentfile = info[0].ToString().Utf8Value();
...
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer") auto worker = new AsyncPromise<lxc_snapshot *, int>(
        deferred,
...
Napi::Value Container::SnapshotRestore(const Napi::CallbackInfo &info)
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") auto snapname = info[0].ToString().Utf8Value();
...
Napi::Value Container::SnapshotDestroy(const Napi::CallbackInfo &info)
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !(info[0].IsString() || info[1].IsBoolean()), "Invalid arguments") bool all = false;
    std::string snapname;
    if (info[0].IsString())
    {
...
Napi::Value Container::AddDeviceNode(const Napi::CallbackInfo &info)
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") auto src_path = info[0].ToString().Utf8Value();
...
Napi::Value Container::AttachInterface(const Napi::CallbackInfo &info)
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") auto dev = info[0].ToString().Utf8Value();
...
Napi::Value Container::Checkpoint(const Napi::CallbackInfo &info)
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer")
        check_deferred(info.Length() <= 0 || !info[0].IsString(), "Invalid arguments") auto directory = info[0].ToString().Utf8Value();
...
{
    auto deferred = Napi::Promise::Deferred::New(info.Env());
    assert_deferred(_container, "Invalid container pointer") auto worker = new AsyncPromise<int>(
        deferred,
